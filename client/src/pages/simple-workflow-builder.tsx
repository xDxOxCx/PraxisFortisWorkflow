...
export default function WorkflowBuilder() {
  ...
  const generateA3ActionPlan = async () => {
    ...
    try {
      ...
      // Generate A3 document content
      const a3Content = generateA3Content(a3Data);
      
      // Create and download A3 document
      const blob = new Blob([a3Content], { type: 'text/plain' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `A3_Action_Plan_${workflowName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.txt`;
      a.click();
      URL.revokeObjectURL(url);

      toast({
        title: "A3 Action Plan Generated",
        description: `A3 document downloaded with ${selectedImprovements.size} improvement(s)`
      });
  ...
  // Generate A3 content
  const generateA3Content = (data: any) => {
...
This A3 action plan addresses workflow optimization opportunities 
identified through AI analysis of the "${workflowName}" process.

PROBLEM STATEMENT:
Current workflow inefficiencies have been identified that impact 
patient flow, staff productivity, and operational costs.

TARGET STATE:
Implement selected improvements to achieve:
...
${selectedImprovements.map((improvement: any, index: number) => `
${index + 1}. ${improvement.title.toUpperCase()}
   Impact: ${improvement.impact} | Time Saved: ${improvement.timeSaved} min
   Category: ${improvement.category}
   Effort Level: ${improvement.effort}
   Priority: ${improvement.priority}/5

   IMPLEMENTATION STEPS:
${improvement.implementationSteps?.map((step: any, stepIndex: number) => `
   ${index + 1}.${stepIndex + 1} ${step.step}
        - Owner: ${step.owner}
        - Timeline: ${step.timeRequired}
        - Resources: ${step.resources.join(', ')}
        - Status: [ ] Not Started [ ] In Progress [ ] Complete
`).join('') || '   No implementation steps defined'}

   SUCCESS METRICS:
   - Time reduction: ${improvement.timeSaved} minutes per cycle
...
- Monthly performance metrics assessment
- Quarterly process improvement evaluation
...
${new Array(50).fill('=').join('')}
Generated by Workflow Optimization Tool
`;
  };
